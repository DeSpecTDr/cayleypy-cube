...
class Searcher:
    ...
    def get_unique_states(self, states, states_bad_hashed):
        ...
        mask2 = torch.concat(
            (
                torch.tensor([True], device=states.device),
    ...
    def get_neighbors(self, states):
        ...
        for i in range(0, states.size(0), self.batch_size):
            ...
            neighbors[i : i + self.batch_size] = torch.gather(
                batch_states.unsqueeze(1).expand(
                    batch_states.size(0), self.n_gens, self.state_size
                ),
                2,
                self.all_moves.unsqueeze(0).expand(
                    batch_states.size(0), self.n_gens, self.state_size
                ),
            )
        return neighbors
    ...
    def apply_move(self, states, moves):
        moved_states = torch.empty(
            states.size(0), self.state_size, device=self.device, dtype=states.dtype
        )
        for i in range(0, states.size(0), self.batch_size):
            moved_states[i : i + self.batch_size] = torch.gather(
                states[i : i + self.batch_size],
                1,
                self.all_moves[moves[i : i + self.batch_size]],
            )
        return moved_states

    def do_greedy_step(self, states, states_bad_hashed, B=1000):
        """Perform a greedy step to find the best neighbors."""
        idx0 = torch.arange(states.size(0), device=self.device).repeat_interleave(
            self.n_gens
        )
        moves = torch.arange(self.n_gens, device=self.device).repeat(states.size(0))
        self.counter[0, 0] += moves.size(0)
        self.counter[0, 1] += 1
        neighbors_hashed = torch.empty(
            moves.size(0), dtype=torch.int64, device=self.device
        )
        for i in range(0, states.size(0), self.batch_size):
            batch_states = states[i : i + self.batch_size]
            neighbors = self.get_neighbors(batch_states).flatten(end_dim=1)
            neighbors_hashed[i * self.n_gens : (i + self.batch_size) * self.n_gens] = (
                state2hash(neighbors, self.hash_vec, self.batch_size)
            )
        idx1 = self.get_unique_hashed_states_idx(neighbors_hashed, states_bad_hashed)
        self.counter[1, 0] += idx1.size(0)
        self.counter[1, 1] += 1
        value = torch.empty(idx1.size(0), dtype=torch.float16, device=self.device)
        for i in range(0, idx1.size(0), self.batch_size):
            batch_states = self.apply_move(
                states[idx0[idx1[i : i + self.batch_size]]],
                moves[idx1[i : i + self.batch_size]],
            )
            value[i : i + self.batch_size] = self.pred_d(batch_states)[0]
        idx2 = torch.argsort(value)[:B]
        self.counter[2, 0] += idx2.size(0)
        self.counter[2, 1] += 1
        next_states = torch.empty(
            idx2.size(0), self.state_size, dtype=states.dtype, device=self.device
        )
    ...
    def check_stagnation(self, states_log):
        ...
        return (
            torch.isin(
                torch.concat(list(states_log)[2:]), torch.concat(list(states_log)[:2])
            )
            .all()
            .item()
        )
    ...
    def get_solution(
        self, state, B=2**12, num_steps=200, num_attempts=10, return_tree=False
    ):
        ...
        for J in range(num_attempts):
            ...
            for j in pbar:
                states, y_pred, moves, idx = self.do_greedy_step(
                    states, states_bad_hashed, B
                )
                if self.verbose:
                ...
                if (states == self.V0).all(dim=1).any():
                    ...
                    states_bad_hashed = torch.concat(
                        (states_bad_hashed, torch.concat(list(states_hash_log)))
                    )
        ...
        if return_tree:
            return (
                moves_seq.flip((0,)),
                J,
                torch.concat((tree_idx.unsqueeze(0), tree_move.unsqueeze(0))).cpu(),
            )
        else:
