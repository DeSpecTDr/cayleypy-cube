...
def main():
    ...
    parser.add_argument(
        "--epochs", type=int, default=256, help="Number of training epochs"
    )
    ...
    parser.add_argument(
        ...
        choices=["relu", "mish"],
        default="relu",
        help="Activation function (relu or mish)",
    )
    parser.add_argument(
        "--use_batch_norm",
        type=bool,
        default=True,
        help="Batch normalization usage (True or False, default True).",
    )
    parser.add_argument(
        "--K_min", type=int, default=1, help="Minimum K value for random walks"
    )
    parser.add_argument(
        "--K_max", type=int, default=30, help="Maximum K value for random walks"
    )
    parser.add_argument(
        "--weights", type=str, default="", help="Path to file with model weights."
    )
    parser.add_argument("--device_id", type=int, default=0, help="Device ID")
    parser.add_argument(
        "--alpha", type=float, default=1, help="TD-learning parameter, avg 1/α steps."
    )
    ...
    parser.add_argument(
        "--hd1", type=int, default=1024, help="Size of the first hidden layer"
    )
    parser.add_argument(
        "--hd2",
        type=int,
        default=256,
        help="Size of the second hidden layer (0 means no second layer)",
    )
    parser.add_argument(
        "--nrd",
        type=int,
        default=4,
        help="Number of residual blocks (0 means no residual blocks)",
    )
    ...
    if len(args.weights) > 0:
        model.load_state_dict(
            torch.load(
                f"weights/{args.weights}.pth", weights_only=True, map_location=device
            )
        )
