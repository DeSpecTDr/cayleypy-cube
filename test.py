...
def main():
    ...
    parser.add_argument(
        "--num_attempts", type=int, default=2, help="Number of allowed restarts."
    )
    parser.add_argument(
        "--num_steps",
        type=int,
    ...
    parser.add_argument(
        "--tests_num", type=int, default=10, help="Number of tests to run"
    )
    ...
    parser.add_argument(
        "--shift", type=int, default=0, help="Shift part of the dataset."
    )
    parser.add_argument(
        "--skip_list",
        type=str,
        help="List of ids, that should be skipped, e.g. '[2, 5]'.",
    )
    parser.add_argument(
        "--return_tree",
        type=int,
    ...
    # Load model info
    with open(
        f"{log_dir}/model_p{int(args.group_id):03d}-t{int(args.target_id):03d}_{args.model_id}.json",
        "r",
    ) as json_file:
    ...
    searcher = Searcher(
        model=model, all_moves=all_moves, V0=V0, device=device, verbose=args.verbose
    )
    ...
    for i, state in enumerate(tests, start=0):
        ...
        if args.return_tree and moves is not None:
            ...
            torch.save(
                tree.cpu(),
                f"{forest_dir}/tree_{args.cube_type}_{args.cube_size}_i{i + args.shift:04d}_B{args.B:08d}_{info['model_id']}.pt",
            )
            torch.save(
                state.cpu(),
                f"{forest_dir}/state_{args.cube_type}_{args.cube_size}_i{i + args.shift:04d}_B{args.B:08d}_{info['model_id']}.pt",
            )
        ...
        if moves is not None:
            ...
            print(
                f"[{timestamp}] Solution {i + args.shift}: Length = {solution_length}"
            )
        else:
